...
export const onIntegrate = async (code: string) => {
  ...
  try {
    ...
    if (integration && integration.integrations.length === 0) {
      ...
      if (token) {
        const insta_id = await getInstagramId(token.access_token);
        if (insta_id) {
          const today = new Date();
          const expire_date = today.setDate(today.getDate() + 60);
          const create = await createIntegration(user.id, token.access_token, new Date(expire_date), insta_id.user_id);
