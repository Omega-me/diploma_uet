...
const Keywords = (props: Props) => {
  const { deleteMuatation, keyword, onKeyPress, onValueChange, variables, isPendingAdd, onClickAddKeyword } = useKeywords(props.id);
  ...
  return (
    <div className="bg-muted flex flex-col gap-y-3 p-3 rounded-xl">
      <p className="text-sm text-muted-foreground">Add words that trigger automations</p>
      <div className="flex flex-wrap justify-start items-center gap-2">
        {automation?.data?.keywords &&
          automation?.data?.keywords.length > 0 &&
          automation?.data?.keywords.map((keyword) => (
            <div key={keyword.id} className="bg-[#141414] flex items-center gap-2 capitalize text-sm text-muted-foreground px-2 py-1 rounded-full">
              <p>{keyword.word}</p>
              <X size={20} className="cursor-pointer" onClick={() => deleteMuatation({ id: keyword.id } as any)} />
            </div>
          ))}
      ...
      <div className="flex justify-between items-center gap-x-2">
        ...
        <AppTooltip text="Add keyword">
          <Button className="bg-gradient-to-br from-[#3352cc] to-[#1c2d70] text-white" onClick={onClickAddKeyword}>
